...
void handleserver::mysql_pool() {
  mysql_connPool = mysql_connection_pool::GetInstance();
  mysql_connPool->init("localhost", m_user, m_passwd, m_databasename,
                       stoi(m_port), m_mysql_num);
  bloom_init();

  //初始化redis连接池
  m_redis_connPool = redis_connection_pool::GetInstance();
  m_redis_connPool->init(m_redis_ip, m_redis_port, m_redis_num);
}

void handleserver::bloom_init() {
  MYSQL *mysql = nullptr;
  mysql_connectionRAII mysqlconn(&mysql, mysql_connPool);
  std::string search = "SELECT * FROM user";
  auto search_res = mysql_query(mysql, search.c_str());
  MYSQL_RES *result = mysql_store_result(mysql);
  if (result == nullptr) {
    printf("mysql_store_result()失败，原因是：%s\n", mysql_error(mysql));
  }
  MYSQL_ROW row;
...
void handleserver::test_redis() {
  ...
  redis_connectionRAII rediscon(&redis, m_redis_connPool);

  redisReply *reply =
      (redisReply *)redisCommand(redis, "set %s %s", "foo", "hello");
  printf("set:%s\n", reply->str);
...
void handleserver::login_user(string str, int cfd, bool &if_login,
                              string &login_name) {

  // 先从连接池中取一个连接
  MYSQL *mysql = NULL;
  mysql_connectionRAII mysqlcon(&mysql, mysql_connPool);
  ...
  if (search_res == 0 && row != 0) {
    ...
    if (info[1] == pass) {
      ...
      for (int i = 0; i < 10; i++) {
        int type =
            rand() % 3; // type为0代表数字，为1代表小写字母，为2代表大写字母
        if (type == 0)
          str1 += '0' + rand() % 9;
        else if (type == 1)
          str1 += 'a' + rand() % 26;
      ...
      std::cout << "随机生成的sessionid为：" << str1.substr(2) << endl;
  ...
  while (1) {
    ...
    // 设定目标的文件描述符
      ...
      if (name_sock_map.find(target) == name_sock_map.end())
        ...
        cout << "源用户" << login_name << "向目标用户" << target_name
             << "发起的私聊即将建立";
    ...
    // 接收到消息，转发
      ...
      if (target_conn == -1) {
        cout << "找不到目标用户" << target_name
             << "的套接字，将尝试重新寻找目标用户的套接字\n";
